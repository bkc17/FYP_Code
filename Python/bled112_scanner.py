...
"""
    ...
    p.set_defaults(port="COM3", baud=256000, interval=0xC8, window=0xC8, display="trpsabd", uuid=[], mac=[], rssi=0, active=False, quiet=False, friendly=False)
                    ...
                    if display:
                        ...
                        for c in options.display:
                            if c == 't':
                                ...
                                temp = "%s" % ''.join(['%02X' % b for b in sender[::-1]])
